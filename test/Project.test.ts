...
test.provider("requires explicit adoption for an existing named project", (stack) =>
  Effect.gen(function* () {
    existingProject = {
      projectId: 42,
...
test.provider("defaults the project directory to the stack working directory", (stack) =>
  Effect.gen(function* () {
...
test.provider("requires explicit adoption for an existing referenced deployment", (stack) =>
  Effect.gen(function* () {
    yield* stack.deploy(
      Project("Backend", {
    ...
    const makeProgram = (shouldAdopt: boolean) =>
      Effect.gen(function* () {
    ...
    const rejected = yield* Effect.exit(stack.deploy(makeProgram(false)));
    expect(Exit.isFailure(rejected)).toBe(true);

    const adopted = yield* stack.deploy(makeProgram(true));
...
test.provider("deploys and memoizes the default production deployment", (stack) =>
  Effect.gen(function* () {
...
test.provider("generates and persists the project name before deploying", (stack) =>
  Effect.gen(function* () {
    const program = Effect.gen(function* () {
...
test.provider("infers development and preview kinds from references", (stack) =>
  Effect.gen(function* () {
    ...
    expect(deploymentEnsures[1]?.expiresAt).toBe(
